...
public class EmailService {
    ...
    @Async
    public void sendMailPasswordChange(String recipient, String verificationCode) throws UnsupportedEncodingException, MessagingException {
        MimeMessage message = mailSender.createMimeMessage();
        message.addRecipients(Message.RecipientType.TO, recipient); // 받는 사람
        message.setSubject("[FIND BY DOG 비밀번호 변경]");
        String text = "안녕하세요, FIND BY DOG 회원님!\n\n"
                + "귀하의 FIND BY DOG 비밀번호 변경을 위한 요청이 접수되었습니다.\n"
                + "아래의 비밀번호를 사용하여 로그인을 완료해 주세요.\n\n"
                + "비밀번호: " + verificationCode + "\n\n"
                + "이 메일에 대해 알지 못하시거나 요청하지 않으셨다면, 귀하의 계정이 위험할 수 있으므로 즉시 저희에게 연락해 주십시오.\n\n"
                + "감사합니다.\n"
                + "FIND BY DOG 팀 드림.";
        message.setText(text, "utf-8");
